...
import { registrationProgressMessage } from "../_lib/registrationHelpers.lib";
export function ImportProgressAction() {
  ...
  if (jobs.length === 1 && reviewId && !reading && !errors.length) return null;
  ...
  return (
    <section className="scan-dashboard" aria-label="파일 분석 진행 목록">
      ...
      {(jobs.length > 1 || (!reading && !reviewId)) && (
